...
<script src="${applicationScope.contextPath}/js/echarts/echarts.min.js"></script>
<script src="${applicationScope.contextPath}/js/echarts/dark.js"></script>
<script src="${applicationScope.contextPath}/js/echarts/china.js"></script>


<script type="text/javascript">
var ws = null;
function connect() {
	var target = '${applicationScope.contextPath}/countlog';
...
var dataQuene = [];
var size = 300;
function processMsg(area) {
	//console.log(area);
	console.log(area);
	//dataQuene.push({name:area,value:100});
	
	var geoCoord = geoCoordMap[area];
	if (geoCoord) {
		dataQuene.push({
            name: area,
            value: geoCoord.concat(area)
        });
    }
	
	
	//超过队列size，舍去
	if(dataQuene.length>size){
		dataQuene.splice(0,dataQuene.length-size);
	}
...
option = {
   ...
   title: {
       text: '舟谱快消',
       subtext: '实时访问监控',
       sublink: 'https://www.zhoupu123.com',
       left: 'center',
       textStyle: {
           color: '#fff'
       }
...
window.setInterval(function(){
	...
	console.log("-------begin-------");
	option.series[0].data = convertData(dataQuene);
	
	var len = dataQuene.length;
	if(len<6){
		option.series[1].data = convertData(dataQuene);
	}else{
		option.series[1].data = convertData(dataQuene).slice(len-6, len-1);
	}
	console.log(len);
	chart.setOption(option);
	console.log("-------end-------");
