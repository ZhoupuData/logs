...
$(function () {
   ...
   var mydatatable = $('#mydatatable').DataTable({
      ...
      "serverSide": true,
      "ajax": {
          "url": "${applicationScope.contextPath}/user"
      },
      //"scrollY": 1000,
      //"scrollX": true,
      //"autoWidth": false,
...
function edit(id){
	 BootstrapDialog.show({
		 title: '编辑',
		 message: $('<div></div>').load('${applicationScope.contextPath}/user/getedituser/'+id),
         buttons: [{
             label: '关闭',
...
function save(callback){
	$('#frm').ajaxSubmit({
	    ...
	    url: '${applicationScope.contextPath}/user/save', // 需要提交的 url
	    //data: data,
...
function remove(id){
	 ...
	 $.ajax({
         type: "delete",
         url: '${applicationScope.contextPath}/user/delete/'+idstr,
         beforeSend: function(){
         	//loading('正在删除，请稍等...');
         },
         complete: function() {
